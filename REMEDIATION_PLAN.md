...
|---|---|---|---|---|---|---|---|---|
| C1 | Choose and enforce a single auth model (Firebase-first for mobile) | `apps/mobile/src/contexts/AuthContext.tsx`, `apps/mobile/src/services/AuthService.ts`, `services/api/src/routes/auth.ts`, `services/api/src/middleware/auth.ts`, `ARCHITECTURE.md` | Firebase is the only mobile auth/session authority; backend JWT mobile flow is removed or isolated | None | GitHub Copilot | Review | Manual auth-flow gate still requires device run with valid Firebase project env; rollback: revert auth route namespace and restore mobile register endpoint call. PR create URL: https://github.com/Hamez-brayo/football-scout/pull/new/remediation/auth-unification | `remediation/auth-unification` |
